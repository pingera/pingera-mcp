...
class TestComponentTools:
    ...
    async def test_list_component_groups_success(self, mock_component_tools):
        ...
        assert len(result_data["data"]["component_groups"]) == 2
        assert result_data["data"]["component_groups"][0]["name"] == "Infrastructure"
        assert result_data["data"]["component_groups"][1]["name"] == "Services"
